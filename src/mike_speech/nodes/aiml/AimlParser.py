...
class AimlHandler(ContentHandler):
    ...
    def __init__(self, encoding=None):
        ...
        self._elemStack = []
        self._currentLearnPattern = []
        self._currentElemStack = []
        self._currentPatternStack = []
    ...
    def _startElement(self, name, attr):
        if name == "aiml":
            ...
            self._elemStack = []
            self._pushWhitespaceBehavior(attr)
        elif name == "pattern" and self._STATE_InsideLearn == True:
            # <template> tags are only legal in the AfterPattern and AfterThat
            # states
            if self._state not in [self._STATE_InsideCategory]:
                raise AimlParserError( "Unexpected <pattern> tag inside <learn>"+self._location() )
            ...
            self._elemStack.append(['pattern',{}])
            self._pushWhitespaceBehavior(attr)
        elif name == "pattern":
            ...
            # <learn> tags are only valid in the InsideCategory state
            if self._state != self._STATE_InsideCategory and self._STATE_InsideLearn:
                raise AimlParserError( "Unexpected <learn> tag "+self._location() )
            self._state = self._STATE_InsideAiml
            self._STATE_InsideLearn = True
            #self._learn._foundDefaultLiStack = self._foundDefaultLiStack
            #self._learn._whitespaceBehaviorStack = self._whitespaceBehaviorStack
            self._currentLearnPattern = self._currentPattern
            self._currentElemStack = self._elemStack
            self._currentPattern = []
            self._elemStack = []
            self._currentPatternStack = []
            ...
            elif self._STATE_InsideLearn == False:
                raise AimlParserError( ( "Unexpected <%s> tag 1st bot " % name)+self._location() )
    ...
    def _endElement(self, name):
        ...
        if name == "aiml":
            ...
            if self._STATE_InsideLearn == False:
                raise AimlParserError( "Unexpected </learn> tag "+self._location() )
            self._state = self._STATE_InsideTemplate
            self._STATE_InsideLearn = False
            key = (self._currentPattern.strip(), self._currentThat.strip(),self._currentTopic.strip())
            key2 = self._currentPatternStack
            val = self.categories[key]
            del self.categories[key]
            text = [key2, val]
            self._elemStack = self._currentElemStack
            self._elemStack[-1].append(["learn", {"xml:space": "default"}, text])
            self._currentPattern = self._currentLearnPattern
            ...
            self._whitespaceBehaviorStack.pop()
            self._currentPatternStack = self._elemStack[-1]
        elif name == "pattern":
            ...
            # Certain tags are allowed inside <pattern> elements.
            if name not in ["bot"] and self._STATE_InsideLearn == False:
                raise AimlParserError( ("Unexpected </%s> tag " % name)+self._location() )
        elif self._state == self._STATE_InsideThat:
            # Certain tags are allowed inside <that> elements.
            if name not in ["bot"]:
